...
Pathoplexus' aim is to provide pathogen sequences from epidemiological and biological settings (sequences which have a sample setting (human, animal, cell, culture) and sample date). In this regard, we do *not* accept sequences intended for vaccine production or patent purposes. Sequences that are clearly vaccine or patent sequences will be [revoked](/docs/how-to/revoke-sequences). We *do* accept vaccine-derived sequences (e.g. sequences obtained after vaccination).
