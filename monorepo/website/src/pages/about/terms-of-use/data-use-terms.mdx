...
---
<br></br>
These Terms were updated on 11 May 2026 and are the current and valid version.
...
Restricted-Use Data is displayed in Pathoplexus with a clear indication that its use is restricted, until the expiration of the Restricted-Use period of up to one year. After the Restricted Use period ends, Restricted-Use Data becomes Open Data and will be released on INSDC.
...
<OutbreakHintBox>
    Treating your entire dataset as a "Background Set" may **not be appropriate in an outbreak
    setting when key outbreak data are Restricted-Use**. During an outbreak, the most important and
    timely data may come from a small number of submitting groups and may still be under Restricted-Use
    protections. In that situation, using those data as background without contacting and involving the
    submitters would not be appropriate — such data should instead be treated as part of your
    [Focal Set](#423-deciding-if-restricted-use-data-is-part-of-a-focal-or-background-set)
