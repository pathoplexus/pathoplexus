...
For some organisms on Pathoplexus, the `host` metadata field is required. Since wastewater samples do not have a clearly defined host organism, please enter `undefined` in the `host` field for organisms where `host` is required.
</details>
