...
You can upload data to any of our supported pathogens by first [creating an account](/docs/how-to/create-account) and then [submitting your sequences](/docs/how-to/upload-sequences), optionally together with the [raw reads](/docs/how-to/upload-raw-reads) they were generated from, on the website or via the API (useful for computational pipelines).
...
If you submit to INSDC, we will pull your data into Pathoplexus, so there's no need to submit it here! Note that we ingest consensus sequences from the INSDC but not raw reads, so if your reads are already there, give their accession in the `Raw reads accession` metadata field when you submit.
