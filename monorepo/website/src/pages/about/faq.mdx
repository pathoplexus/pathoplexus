...
Pathoplexus currently does not accept uploads of raw sequencing reads. However, you may submit consensus sequences derived from wastewater sequencing data.

If you do, please enter the appropriate standardized term in the `environmentalMaterial` metadata field when submitting your data (e.g., waste water [ENVO:00002001]).
For some organisms on Pathoplexus, the `host` metadata field is required.
Should the host organism for your wastewater sample be unknown, you may enter 'unidentified' in the `host` field.
</details>
