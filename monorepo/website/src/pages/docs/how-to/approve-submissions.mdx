...
After submitting sequences, you'll be taken to a page showing the progress of processing every sequence. For each sequence, it will show whether it's awaiting processing, being processed, or has finished processing.
...
You can only approve and release sequences that have yellow or green checkmarks (no warnings or errors, or only warnings) - you cannot approve and release sequences with errors.
