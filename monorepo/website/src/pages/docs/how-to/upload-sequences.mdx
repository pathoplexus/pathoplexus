...
Similarly, raw reads files will be associated to their corresponding consensus sequence via the `fileIds` metadata field. This field should contain a space-separated list of file names indicating which raw reads files belong to which sequence. Raw reads for the full submission should be uploaded as a single (possibly nested) folder containing raw reads files for all sequences (see [here](/docs/how-to/attach-additional-files) for a detailed description of how to attach raw reads files).
...
The most convenient way to upload raw reads that are associated with your sequences is to included them directly in your submission using the instructions on this page.

However, it is also possible to upload your raw reads to the INSDC yourself and link them to your consensus sequence on Pathoplexus (if you have a specific Bioproject or Biosample you want to use, for example). In that case, you can submit your raw reads directly through the [INSDC submission portal](https://ena-docs.readthedocs.io/en/latest/submit/reads.html) and link them to sequences in Pathoplexus. 

To ensure your raw reads are properly linked to your consensus sequence in the INSDC, both the consensus sequence and the raw reads should be associated with the same BioSample and BioProject at the time of submission.


#### Submission Scenarios:
- Submitting the Consensus Sequence and Raw Reads Together (via Pathoplexus, following instructions above): This is the recommended way of submitting a consensus sequence with its associated raw reads to Pathoplexus and will ensure your raw reads are immediately linked to your consensus sequence in the INSDC.
