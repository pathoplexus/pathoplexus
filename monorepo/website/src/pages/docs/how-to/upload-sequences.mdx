...
- Raw reads files in `fastq.gz` format (optional).
...
The metadata and sequence data files can be compressed: accepted formats here are .zst, .gz, .zip and .xz. Raw reads files must be compressed, and only the .gz format is accepted (to facilitate submission to INSDC).
