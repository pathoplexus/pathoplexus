...
Similarly, raw reads files will be associated to their corresponding consensus sequence via the `files.rawReads` metadata field. This field should contain a space-separated list of file names indicating which raw reads files belong to which sequence. Raw reads for the full submission should be uploaded as a single folder containing raw reads files for all sequences (see [here](/docs/how-to/attach-additional-files) for a detailed description of how to attach raw reads files).
