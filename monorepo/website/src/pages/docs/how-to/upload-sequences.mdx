...
The process of submitting sequences to Pathoplexus consists of three sequential steps; sequence upload, editing, and approval.
The first step is sequence upload and requires you to have [created an account](create-account) and to be part of a [group](create-group).
If you already have an account and belong to more than one group, ensure that the approraite group your currently submitting sequences for is chosen from the 
drop down menu on the top left before proceeding with the submission process. Otherwise, the sequences will be assigned to the previous group used on your last
submission.
...
<InfoBox>
If you have selected `Restricted` on the terms of use for your sequences, a time limit for the restriction will be automatically set to 1 year from the date on which your submitting the sequences. You can customise this to an ealier date of your preference by using the `change date` button under restricted use before proceeding to submission. 
This action can also be performed even if the sequences were already submitted and you wish to make the sequences `open` before the restriction period ends. However, this would requires you to be logged in as someone with authorisation.
</InfoBox>
