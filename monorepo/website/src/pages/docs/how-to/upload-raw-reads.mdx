...
If the raw reads for a sequence are already available in the INSDC, do not upload them again. Instead, give their accession in the `insdcRawReadsAccession` ('Raw reads accession') metadata field, and we will link to them rather than submitting a second copy.
