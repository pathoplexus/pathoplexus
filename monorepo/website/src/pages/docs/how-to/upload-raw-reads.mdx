...
<InfoBox>
Raw reads files must be submitted as gzip-compressed FASTQ, named `.fastq.gz` or `.fq.gz`. Uncompressed FASTQ and other compression formats such as `.bz2` or `.zst` are rejected. You can provide either a single file or a pair of files for consensus sequences generated from single-end or paired-end reads, respectively. Note that if you are submitting paired-end reads, they **must** be provided as two separate files: one with the forward reads and the other with the reverse. Interleaved files are not supported.
</InfoBox>
