...
- **Add or remove raw reads**: You can upload additional raw reads files or discard raw reads files currently associated with the sequence
...
By default, revising the metadata or sequence data will **not** affect the raw reads files attached to your submissions. This means that if you don't want to update the raw reads data of your submission you do not need to re-upload the raw reads files and do not need to perform any additional action besides making the metadata or sequence edits you want. Just ensure to leave the `files.rawReads` column exactly as it was when you downloaded the original data.
...
- **Adding additional raw reads files:** in your metadata file, find the entry for which you want to add a raw reads file. Add the name of the file to the `files.rawReads` field (see [here for a detailed description](/docs/how-to/attach-additional-files)), and upload the file via the 'Upload folder' or 'Upload files' functionality during submission.
- **Discarding previously uploaded raw reads files:** in your metadata file, find the entry for which you want to delete a raw reads file. Go to the `files.rawReads` field and remove the name of the file you wish to delete. The name will be shown as `name:identifier`, where `identifier` is a random string of letters and numbers, used for internal identification. Please ensure to remove to full `name:identifier` entry from your metadata file in order to delete the raw reads.
- **Replacing a previously uploaded raw reads file with another one:** this can be done through a combination of the two approaches outlined above: first, identify the `name:identifier` of the file you want to replace, replace it with the name of the new file, and upload the corresponding file via the 'Upload folder' or 'Upload files' functionality during submission.
