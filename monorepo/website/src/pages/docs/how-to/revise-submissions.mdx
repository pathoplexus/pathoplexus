...
- **Discarding previously uploaded raw reads files:** in your metadata file, find the entry for which you want to delete a raw reads file. Go to the `files.rawReads` field and remove the name of the file you wish to delete. The name will be shown as `name:identifier`, where `identifier` is a random string of letters and numbers. The meaning of the `identifier` part is not relevant here, but please ensure to remove to full `name:identifier` entry from your metadata file.
