...
It needs to include an `accession` column, which includes the Pathoplexus accessions assigned at initial submission (without the version number, e.g., `PPX_0049AAG` not `PPX_0049AAG.1`).
