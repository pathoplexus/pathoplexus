...
**Important:** The `accession` column should contain the accession **without** the version number. For example, use `PPD_0049AAG` rather than `PPD_0049AAG.1`. (See [Accessions](/docs/concepts/accession) for more information on accession format.)
...
It needs to include an `accession` column, which includes the Pathoplexus accessions assigned at initial submission (without the version number, e.g., `PPD_0049AAG` not `PPD_0049AAG.1`).
