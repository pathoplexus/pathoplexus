...
In ENA, Projects contain general information on your group and the organism being sequenced. We create one Project per each group and organism. In ENA Samples contain metadata information and Assemblies contain the actual sequences. We create one sample and one assembly object per sequence, and one experiment and one run object per sequence that has raw reads attached.

## Citing your Sequences

If you would like to cite your sequences in a publication you can use your Bioproject accession (this will start with `PRJ`), Biosample accession (this will start with `SAM`), Genome Assembly accession (starting with `GCA`) and, for raw reads, the Run accession (starting with `ERR`).
...
| MOLECULETYPE                | NaN (set based on organism)           |
| RUN_REF                     | insdcRawReadsAccession                |
...
| INSERT_SIZE            | sequencingLibrarySelection (only if reads are paired) |
