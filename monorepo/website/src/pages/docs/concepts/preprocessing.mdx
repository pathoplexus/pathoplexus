...
- Raw reads:
  - Screening for reads that are likely to be human. Files in which an excessive number of such reads is detected are rejected, so reads should be [dehosted](/docs/how-to/upload-raw-reads#dehosting-host-read-removal) before submission.
